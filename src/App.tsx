    ...
    <AppShell active="Exceptions" goLanding={goLanding} goDashboard={goDashboard}>
      <main className="case-page" id="main-content">
        ...
        <section className="case-layout">
          <article className="case-path"><header><span>Transaction path</span><Badge variant="secondary">Rule verified</Badge></header><ol>{pathEvents.map((event, index) => { const Icon = [FileCheck2, BadgeIndianRupee, Banknote, BadgeIndianRupee, Landmark, Building2][index]; return <li key={event.reference}><button className={`${event.state} ${selected === index ? "selected" : ""}`} onClick={() => setSelected(index)} aria-pressed={selected === index}><span><Icon aria-hidden="true" /></span><div><strong>{event.label}</strong><code>{event.reference}</code>{event.detail ? <small>{event.detail}</small> : null}</div><b>{event.amount}</b></button></li>})}</ol><div className="case-path__alert"><AlertTriangle aria-hidden="true" /><p><strong>First divergence</strong>The actual bank credit is ₹3.40 below the expected settlement.</p></div></article>
