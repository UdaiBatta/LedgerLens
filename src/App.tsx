...
function AppShell({ active, children, goLanding, goDashboard, goExceptions, goMoneyGraph, goAuditLog }: { active: string; children: React.ReactNode; goLanding: () => void; goDashboard: () => void; goExceptions: () => void; goMoneyGraph?: () => void; goAuditLog?: () => void }) {
  const [identity, setIdentity] = useState<{ organization: string; name: string; role: string } | null>(null)
  useEffect(() => { getIdentity().then(setIdentity).catch(() => setIdentity(null)) }, [])
    ...
    <div className="app-shell">
      ...
      <aside className="app-sidebar app-sidebar--rich">
        ...
        <SidebarNav
          ...
          workspaceName={identity?.organization ?? "LedgerLens"}
          workspacePlan={identity?.role ?? "Sign in required"}
        />
...
function Dashboard({ goLanding, openCase, goExceptions, goMoneyGraph, goAuditLog }: { goLanding: () => void; openCase: (publicId: string) => void; goExceptions: () => void; goMoneyGraph: (publicId: string) => void; goAuditLog: () => void }) {
  ...
  return (
    <AppShell active="Overview" goLanding={goLanding} goDashboard={() => undefined} goExceptions={goExceptions} goMoneyGraph={firstCaseId ? () => goMoneyGraph(firstCaseId) : undefined} goAuditLog={goAuditLog}>
      <main className="dashboard" id="main-content">
        <header className="dashboard__heading"><div><p>Reconciliation overview</p><h1>Financial truth, in one place.</h1><span>Live data from the Django reconciliation API</span></div><Button onClick={goExceptions}>Review exceptions <ArrowRight data-icon="inline-end" aria-hidden="true" /></Button></header>
        ...
        {metrics ? <section className="metric-rail" aria-label="Reconciliation summary"><div><span>Captured value</span><strong>{formatMoney(metrics.captured_amount_minor, metrics.currency)}</strong><small>{metrics.movement.find((stage) => stage.record_type === "payment")?.record_count ?? 0} payment records</small></div><div><span>Unexplained value</span><strong>{formatMoney(metrics.unexplained_amount_minor, metrics.currency)}</strong><small>Across {metrics.open_case_count} open cases</small></div><div><span>Matched cases</span><strong>{metrics.matched_case_count} / {metrics.case_count}</strong><small>Generated by deterministic rules</small></div><div><span>Source health</span><strong>{metrics.fresh_source_count} / {metrics.source_count}</strong><small>Sources with records received in 24h</small></div></section> : null}
        <section className="dashboard-grid">
          ...
          <Briefing cases={cases} openCase={() => firstCaseId && openCase(firstCaseId)} goExceptions={goExceptions} />
        </section>
      </main>
    </AppShell>
  )
...
function EvidenceTrail({ publicId, casesLoading, openCase, openGraph }: { publicId?: string; casesLoading: boolean; openCase: () => void; openGraph: () => void }) {
  const [response, setResponse] = useState<{ id: string; data: ReconciliationCaseDetail | null; error: string } | null>(null)
  const reconciliationCase = response?.id === publicId ? response?.data : null
  const error = response?.id === publicId ? response?.error : ""
  ...
  useEffect(() => {
    ...
    let active = true
    getCase(publicId)
      .then((data) => { if (active) setResponse({ id: publicId, data, error: "" }) })
      .catch((requestError: Error) => { if (active) setResponse({ id: publicId, data: null, error: requestError.message }) })
    return () => { active = false }
  }, [publicId])
    ...
    <article className="trace-panel">
      ...
      {reconciliationCase ? <>
        <div className="trace-summary">
          <div><span>Expected at first check</span><strong>{reconciliationCase.amounts_known ? formatMoney(reconciliationCase.expected_amount_minor, currency) : "Awaiting evidence"}</strong></div>
          <div><span>{readableLabel(reconciliationCase.first_break_record?.record_type ?? "actual")}</span><strong>{reconciliationCase.amounts_known ? formatMoney(reconciliationCase.actual_amount_minor, currency) : "Awaiting evidence"}</strong></div>
          <div className="trace-summary__variance"><span>Unexplained variance</span><strong>{reconciliationCase.amounts_known ? `${difference < 0 ? "−" : "+"}${formatMoney(Math.abs(difference), currency)}` : "Not established"}</strong></div>
        </div>
        <ol className="trace-flow">
          {checkpoints.map((record, index) => {
            ...
            return (
              <li key={record.id}>
                {previous ? <span className={`trace-link ${linkIsBreak ? "trace-link--broken" : ""}`} aria-hidden="true"><b>{linkIsBreak && reconciliationCase.amounts_known ? `${difference < 0 ? "−" : "+"}${formatMoney(Math.abs(difference), currency)}` : "REFERENCE"}</b></span> : null}
                <button className={`trace-card ${isBreak ? "trace-card--break" : ""}`} onClick={() => setSelectedRecord(record)}>
                  ...
                  <em><i aria-hidden="true" />{isBreak ? "First break" : TRAIL_CHECKPOINT_LABELS[record.record_type] ?? "Source record"}</em>
                </button>
              </li>
            )
          })}
        ...
        {breakIndex >= 0 ? (
          <div className="trace-break">
            ...
            <div>
              <strong>{readableLabel(reconciliationCase.first_break_record?.record_type ?? "Record")} {reconciliationCase.amounts_known ? `differs by ${formatMoney(Math.abs(difference), currency)}.` : "requires more evidence."}</strong>
              <p>Inspect the deterministic checks to distinguish a measured difference from missing evidence.</p>
            </div>
          </div>
        ) : null}
...
function Briefing({ cases, openCase, goExceptions }: { cases: ReconciliationCaseSummary[]; openCase: () => void; goExceptions: () => void }) {
...
function CaseDetail({ publicId, goLanding, goDashboard, goExceptions, goMoneyGraph, goAuditLog }: { publicId: string; goLanding: () => void; goDashboard: () => void; goExceptions: () => void; goMoneyGraph: (publicId: string) => void; goAuditLog: () => void }) {
  ...
  async function assignToMe() {
    ...
    try {
      setReconciliationCase(await assignCase(reconciliationCase.public_id))
    } catch (requestError) {
  ...
  return (
    <AppShell active="Exceptions" goLanding={goLanding} goDashboard={goDashboard} goExceptions={goExceptions} goMoneyGraph={() => goMoneyGraph(publicId)} goAuditLog={goAuditLog}>
...
function MoneyGraphPage({ publicId, goLanding, goDashboard, goExceptions, goCase, goAuditLog }: { publicId: string; goLanding: () => void; goDashboard: () => void; goExceptions: () => void; goCase: (publicId: string) => void; goAuditLog: () => void }) {
  ...
  return (
    <AppShell active="Money Graph" goLanding={goLanding} goDashboard={goDashboard} goExceptions={goExceptions} goAuditLog={goAuditLog}>
      <main className="money-graph-page" id="main-content">
        ...
        {evidenceGraph ? <>
          ...
          <div className="money-graph-legend"><span><i className="exact" />Exact reference match</span><span><i className="inferred" />Candidate — not verified</span><span><i className="break" />First divergence</span></div>
...
function AuditLogPage({ goLanding, goDashboard, goExceptions, goMoneyGraph, goCase }: { goLanding: () => void; goDashboard: () => void; goExceptions: () => void; goMoneyGraph: (publicId: string) => void; goCase: (publicId: string) => void }) {
  ...
  const [filter, setFilter] = useState<"All" | "Investigator runs" | "Control events">("All")
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState("")
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (filter === "Control events") return entries.filter((entry) => entry.event_type !== "agent_run")
    return entries
  }, [entries, filter])
    ...
    <AppShell active="Audit Log" goLanding={goLanding} goDashboard={goDashboard} goExceptions={goExceptions} goMoneyGraph={firstCasePublicId ? () => goMoneyGraph(firstCasePublicId) : undefined} goAuditLog={() => undefined}>
      <main className="audit-log-page" id="main-content">
        <header className="audit-log-page__heading">
          <div><span>Audit log</span><h1>Reconciliation and operator history</h1><p>Persisted events record reconciliation runs, assignments, workflow changes, and investigator activity.</p></div>
          <div className="filters" aria-label="Filter audit log">{(["All", "Investigator runs", "Control events"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={filter === item ? "active" : ""} aria-pressed={filter === item}>{item}</button>)}</div>
        </header>
...
function ExceptionsPage({ goLanding, goDashboard, goMoneyGraph, goAuditLog, openCase }: { goLanding: () => void; goDashboard: () => void; goMoneyGraph: (publicId: string) => void; goAuditLog: () => void; openCase: (publicId: string) => void }) {
  ...
  return (
    <AppShell active="Exceptions" goLanding={goLanding} goDashboard={goDashboard} goExceptions={() => undefined} goMoneyGraph={firstCaseId ? () => goMoneyGraph(firstCaseId) : undefined} goAuditLog={goAuditLog}>
      <main className="dashboard" id="main-content">
        <header className="dashboard__heading"><div><p>Reconciliation exceptions</p><h1>Every case that needs a look.</h1><span>{cases.length} cases across every deterministic check</span></div></header>
        {loading ? <p className="api-state" role="status">Loading reconciliation data…</p> : null}
        {error ? <p className="api-state api-state--error" role="alert">{error} Start Django on port 8000 and seed the demo cases.</p> : null}
        {!loading && !error ? <Exceptions rows={filteredCases} filter={filter} setFilter={setFilter} openCase={openCase} hideHeader /> : null}
      </main>
    </AppShell>
  )
}
...
export default function App() {
  ...
  const [screen, setScreen] = useState<Screen>(() => {
    ...
    if (window.location.hash === "#exceptions") return "exceptions"
